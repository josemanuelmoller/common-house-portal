/**
 * Loops ingestor HTTP entrypoint.
 *
 * Reads active loops from the legacy Supabase `loops` table and emits
 * ActionSignals into action_items. See docs/NORMALIZATION_ARCHITECTURE.md §11.
 */
...
async function handle(req: NextRequest) {
  ...
  const maxItems = maxRaw ? Math.max(1, Math.min(500, Number(maxRaw) || 200)) : undefined;
  const input: IngestInput = { mode, since, dryRun, maxItems };
  try {
    const r = await runLoopsIngestor(input);
