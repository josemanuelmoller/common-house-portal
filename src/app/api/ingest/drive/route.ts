/**
 * Drive ingestor HTTP entrypoint.
 * See docs/NORMALIZATION_ARCHITECTURE.md §11 Drive.
 */

import { NextRequest, NextResponse } from "next/server";
import { runDriveIngestor } from "@/lib/ingestors/drive";
...
export const POST = withRoutineLog("ingest-drive", handle);
...
async function handle(req: NextRequest) {
  ...
  try {
    const r = await runDriveIngestor(input);
