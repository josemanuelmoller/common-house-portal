...
function LearningRow({ projectId, learning }: { projectId: string; learning: ProjectLearningItem }) {
  ...
  const observed = learning.status === "observed";
  const inReview = learning.status === "review";
  const confirmed = learning.transferability === "confirmed";
  // Explicit flow: Observed → Review → Confirmed → Promote. Nothing auto-promotes.
  const stage = promoted ? "Confirmed → Promoted"
    : inReview && confirmed ? "Confirmed"
    : inReview ? "In review"
    : "Observed";
  return <article className="py-3" style={{ borderBottom: "1px solid var(--hall-line-soft)" }}><div className="flex flex-wrap items-start gap-3"><div className="flex-1 min-w-[230px]"><p className="text-[10px] uppercase tracking-[0.07em]" style={{ fontFamily: "var(--font-hall-mono)", color: "var(--hall-muted-2)" }}>{learning.area || "Implementation"} · {learning.learningType.replaceAll("_", " ")} · {stage}</p><p className="mt-1 text-[13px] font-semibold">{learning.title}</p><p className="mt-1 text-[11px] leading-relaxed" style={{ color: "var(--hall-ink-3)" }}>{learning.observation}</p>{learning.implication && <p className="mt-1 text-[11px]" style={{ color: "var(--hall-muted-2)" }}>So what: {learning.implication}</p>}{learning.linkedEntities.length > 0 && <div className="mt-1.5 flex flex-wrap gap-1.5">{learning.linkedEntities.map((e) => <Link key={`${e.entityType}:${e.entityId}:${e.relation}`} href={`/admin/entities/${e.entityType}/${e.entityId}`} className="hall-chip-outline" style={{ textDecoration: "none" }}>{e.relation}: {e.entityName}</Link>)}</div>}{message && <p className="mt-1 text-[10px]" style={{ color: failed ? "var(--hall-danger)" : "var(--hall-muted-2)" }}>{message}</p>}</div>{promoted ? <span className="hall-chip-outline">Promoted ✓</span> : <div className="flex gap-2">{observed && <button className="hall-btn-ghost" type="button" disabled={busy} onClick={() => void update({ status: "review" })}>Mark for review</button>}{inReview && !confirmed && <button className="hall-btn-ghost" type="button" disabled={busy} onClick={() => void update({ transferability: "confirmed" })}>Confirm</button>}{inReview && confirmed && <button className="hall-btn-primary" type="button" disabled={busy} onClick={() => void promote()}>Promote →</button>}</div>}</div></article>;
}
