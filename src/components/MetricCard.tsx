...
export function MetricCard({ label, value, icon, color = "default", sub }: Props) {
  ...
  return (
    <div className={`rounded-2xl border p-5 flex flex-col gap-1 bg-white ${
      isAlert ? "border-red-200" : "border-[#E0E0D8]"
    }`}>
      {icon && <span className="text-lg">{icon}</span>}
      <span className={`text-3xl font-bold tracking-tight mt-1 ${
        isAlert ? "text-red-600" : "text-[#131218]"
      }`}>
        {value}
      </span>
      <span className="text-xs font-medium text-[#131218]/50 uppercase tracking-widest">{label}</span>
      {sub && <span className="text-xs text-[#131218]/30">{sub}</span>}
    </div>
  );
}
