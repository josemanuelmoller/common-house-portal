      ...
      <div className="divide-y divide-[#EFEFEA]">
        {nextSteps.map((step) => (
          <div key={step.id} className="px-6 py-4 flex items-start gap-4">
            <span className="w-5 h-5 rounded-full bg-[#EFEFEA] flex items-center justify-center text-[10px] font-bold text-[#131218]/40 shrink-0 mt-0.5">
              {step.order}
            </span>
            <div className="flex-1">
              <p className="text-sm text-[#131218] leading-relaxed">
                {step.label}
              </p>
            </div>
            {step.owner && (
              <span className="text-[10px] font-bold text-[#131218]/30 uppercase tracking-widest shrink-0 mt-0.5">
                {step.owner}
              </span>
            )}
          </div>
