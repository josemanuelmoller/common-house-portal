      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-[#EFEFEA]">
        {materials.map((mat) => (
          <a
            ...
            className="flex items-start gap-4 px-6 py-4 hover:bg-[#EFEFEA]/60 transition-colors border-b border-[#EFEFEA] last:border-b-0 sm:last:border-b sm:odd:border-b sm:[&:nth-last-child(-n+2)]:border-b-0"
          >
            <span className="text-base text-[#131218]/20 shrink-0 mt-0.5">
              {platformIcon(mat.platform)}
            </span>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-[#131218] truncate">
                {mat.title}
              </p>
              {mat.description && (
                <p className="text-xs text-[#131218]/40 mt-0.5 leading-snug line-clamp-2">
                  {mat.description}
                </p>
              )}
              {mat.addedDate && (
                <p className="text-[10px] text-[#131218]/25 font-bold uppercase tracking-widest mt-1.5">
                  Added {mat.addedDate}
                </p>
              )}
            </div>
            <span className="text-[#131218]/20 text-xs shrink-0 mt-0.5">↗</span>
          </a>
        ))}
      </div>
    </div>
