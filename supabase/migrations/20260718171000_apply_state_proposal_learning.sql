-- Portal 2.0 — apply_state_proposal: give accepted learnings a stale_after and
-- record applied_learning_id (so acceptance can resolve the learning's entity links).
-- CREATE OR REPLACE only; grants/security unchanged.
...
begin
  ...
  if v_prop.status <> 'pending' then
    raise exception 'proposal % is not pending (status=%)', p_proposal_id, v_prop.status using errcode = '55000';
  end if;
  ...
  if v_kind = 'add_item' then
    ...
    select * into v_item from public.project_state_items
     where id = v_prop.target_item_id and project_id = v_prop.project_id for update;
    ...
    insert into public.project_states (
      project_id, current_summary, current_phase, current_focus, health, updated_by, last_state_change_at, updated_at
    ) values (
      v_prop.project_id, nullif(trim(v_payload->>'current_summary'),''), nullif(trim(v_payload->>'current_phase'),''),
      nullif(trim(v_payload->>'current_focus'),''), coalesce(v_health, 'unknown'), p_actor, now(), now()
    )
    on conflict (project_id) do update set
      ...
      updated_by           = p_actor, last_state_change_at = now(), updated_at = now()
    ...
    insert into public.project_learning_items (
      ...
      status, transferability, confidence, source_refs, last_seen_at, stale_after, created_by, updated_by
    ) values (
      v_prop.project_id, v_learning_type, nullif(trim(v_payload->>'area'),''), trim(v_payload->>'title'),
      trim(v_payload->>'observation'), nullif(trim(v_payload->>'implication'),''),
      'observed', 'project', v_prop.confidence, coalesce(v_prop.source_refs,'{}'), now(), now() + interval '45 days', p_actor, p_actor
  ...
  values (v_prop.project_id, 'system_refresh', p_actor, v_snapshot,
  ...
  update public.project_state_proposals set
    ...
    applied_learning_id = v_applied_learning_id,
